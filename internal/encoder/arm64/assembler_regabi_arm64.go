...
func (self *Assembler) check_size_r(r obj.Addr, d int) {
	self.check_size_rl(jit.Ptr(_RP, 0).Reg, jit.RegShift(r, 0))
}
...
// String encoding routine - this is a complex function that needs to be implemented
func (self *Assembler) encode_string(doubleQuote bool) {
	// Implementation would be similar to AMD64 version but adapted for ARM64
	// This is a placeholder that would need full implementation
	self.Emit("MOVD", jit.Ptr(_SP_p, 8), _TEMP0)            // LDR X0, [SP_p, #8]
	self.Emit("CMP", _TEMP0, _ZR)                           // CMP X0, XZR
	self.Sjmp("B.EQ", "_str_empty_{n}")                      // B.EQ _str_empty_{n}

	// String encoding logic would go here
	...
	// Copy string (simplified)
	self.Emit("MOVD", _TEMP0, _ARG1)                        // MOV X0, X1 (src)
	self.Emit("MOVD", _TEMP1, _ARG2)                        // MOV X1, X2 (len)
