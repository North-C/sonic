...
func BenchmarkARM64JIT_Encode_Simple(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, err := sonic.Marshal(benchmarkSimpleData)
		if err != nil {
...
func BenchmarkARM64JIT_Encode_Complex(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, err := sonic.Marshal(benchmarkComplexData)
		if err != nil {
			b.Fatalf("Encode error: %v", err)
		}
	}
...
func BenchmarkARM64JIT_Encode_Nested(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, err := sonic.Marshal(benchmarkNestedData)
		if err != nil {
			b.Fatalf("Encode error: %v", err)
		}
	}
...
func BenchmarkARM64JIT_Encode_LargeArray(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, err := sonic.Marshal(benchmarkLargeArray)
		if err != nil {
			b.Fatalf("Encode error: %v", err)
		}
	}
...
func BenchmarkARM64JIT_Encode_LargeMap(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, err := sonic.Marshal(benchmarkLargeMap)
		if err != nil {
			b.Fatalf("Encode error: %v", err)
		}
	}
...
func TestARM64JIT_Stack_Usage(t *testing.T) {
	...
	t.Logf("Stack usage analysis passed")
}

// Cache performance tests using pretouch
func TestARM64JIT_Cache_Performance(t *testing.T) {
	// Test pretouch functionality
	err := sonic.Pretouch(reflect.TypeOf(benchmarkComplexData))
	if err != nil {
		t.Fatalf("Pretouch error: %v", err)
	}

	// Test encode after pretouch
	start := time.Now()
	for i := 0; i < 100; i++ {
		_, err := sonic.Marshal(benchmarkComplexData)
		if err != nil {
	...
	t.Logf("Encode performance after pretouch: %v total, %v per operation",
		encodeTime, encodeTime/100)
}